...
### 🚪 دروازه ۵: نسخه‌بندی معنایی، انتشار پایدار و پالایش شاخه `main` (Lean Release Gate)
- **فایل گردش کار:** `ci/workflows/release.yml` (آماده انتقال به `.github/workflows/release.yml`)
- **معیار پذیرش:**
  - **پالایش فایل‌های غیرضروری (Lean Main Policy):** انتقال از `dev` به `main` فقط شامل فایل‌های مورد نیاز پروداکشن، بسته‌های اجرایی، پیکربندی استقرار و اسناد عمومی است تا از افزایش غیرضروری حجم شاخه اصلی جلوگیری شود.
  - **برچسب‌گذاری نسخه‌ها:** ثبت تگ رسمی مطابق استاندارد SemVer (`vMAJOR.MINOR.PATCH`).
  - **تولید خودکار Release Notes:** تولید لیست تغییرات و ویژگی‌های افزوده شده بر اساس کامیت‌های ادغام‌شده.
  - **آماده‌سازی کانتینرهای داکر:** بیلد و انتشار ایمیج‌های سبک و ایزوله برای استقرار روی سرورهای عملیاتی.
...
- [ ] چک‌های خودکار CI (`QA Gate`) همگی سبز و موفق هستند (`lint`, `typecheck`, `test`, `build`).
- [ ] اسکن امنیتی (`Security Gate`) بدون خطا پاس شده و هیچ کلید حساسی وجود ندارد.
- [ ] هیچ فایل حساس نظیر `.env`، فایل‌های باینری سنگین یا سکرت‌ها کامیت نشده است.
- [ ] **برای PRهای منتهی به `main` (Release PR):** اعمال **خط‌مشی Lean Main** (بررسی حجم فایل‌ها، عدم وجود فایل‌های غیرضروری یا پیش‌نویس‌های حجیم بلااستفاده).
- [ ] بازبینی کد انجام شده و تمام ابهامات در کامنت‌ها حل‌وفصل شده است.
